(function () {
  ...
  const SUPABASE_PROJECT_REF = "ctqwiuenollyncwnstzz";
  ...
  const formatCents = (cents, currency = DEFAULT_CURRENCY) => {
    const amount = (Math.max(0, cents) / 100).toFixed(2).replace(".", ",");
  ...
  const beginCheckout = async () => {
    try {
      const cart = loadCart();
      cart.items = cart.items.filter((i) => VALID_VARIANT_IDS.has(String(i.variantId || "")));
      saveCart(cart);
      ...
      const checkoutUrl = await createShopifyCheckoutUrl(cart);
  ...
  const FALLBACK_SUPABASE_URL = "https://ctqwiuenollyncwnstzz.supabase.co";
  const FALLBACK_SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImN0cXdpdWVub2xseW5jd25zdHp6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI5NzY1ODMsImV4cCI6MjA4ODU1MjU4M30.ZHs03dUICUZ0FrKdQKGQa-h8iYYSa7plDKkunb_RFqg";
  ...
  const addToCart = ({ variantId, productName, variantName, price, imageUrl, quantity = 1 }) => {
    const cart = loadCart();
    upsertItem(cart, {
      variantId,
      productName,
