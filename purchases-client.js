...
const ensurePurchasesStyles = () => {
  ...
  style.textContent = `
    ...
    :root{
      ...
      --accent-rgb:13,97,243;
      --light-accent-color:rgba(13,97,243,.15);
...
const renderPurchases = (rows, { userReviewsByOrderId } = {}) => {
  ...
  const getProductImageSrc = (title) => {
    const t = String(title || "").toLowerCase();
  ...
  const renderOrderedItemsCard = (p) => {
    ...
    const blocks = (items.length ? items : [null])
      .map((li) => {
        ...
        const itemImage = getProductImageSrc(itemName);
  ...
  const renderOrderBlock = (p) => {
    ...
    const paymentMethod = "Shopify";
    const amount = formatMoney(p?.total_price, p?.currency || "EUR");
          ...
          <div class="bg-background rounded-lg flex flex-col">
            ...
            <div class="p-4 flex flex-col gap-1">
              ...
              <div class="flex flex-col mt-4 gap-1">
                <div class="text-xs">Thank You for your purchase! Join to our Discord Community Server -> https://discord.gg/kylogg</div>
              </div>
            </div>
          </div>
        `;
...
const ensureCustomerRole = async (supabase, user, { currentUsername, currentDisplayName, purchases } = {}) => {
  ...
  if (email === "kyloprojects@outlook.com" || metaUsername === "admin") return false;
...
const main = async () => {
  ...
  const { data, error } = await supabase
    .from("purchases")
    .select("id, user_id, purchaser_email, shopify_order_id, order_name, financial_status, currency, total_price, created_at, processed_at, line_items, license_keys, raw")
  ...
  showList(renderPurchases(rows, { userReviewsByOrderId: reviewsByOrder }));
